...
const FibonacciBackground: React.FC<FibonacciBackgroundProps> = ({ 
  ...
  useEffect(() => {
    ...
    const buildSpiralPoints = () => {
      ...
      const sizeFactor = diagonalLength * 1.05;
      ...
      return { primaryColor, secondaryColor, primary, secondary };
    };
    ...
    const render = (tMs: number) => {
      ...
      const epsilon = mode.motion === 'uniform' ? 0.007 : 0.01;
      ...
      for (let i = 0; i < cached.primary.length; i++) {
        ...
        const px = (p.x0 - canvas.width / 2) * pScale + canvas.width / 2;
        const py = (p.y0 - canvas.height / 2) * pScale + canvas.height / 2;

        const sx = (s.x0 - canvas.width / 2) * sScale + canvas.width / 2;
        const sy = (s.y0 - canvas.height / 2) * sScale + canvas.height / 2;

        linePointsPrimary.push({ x: px, y: py });
        linePointsSecondary.push({ x: sx, y: sy });

        const pSize = calculateDotSize(p.normalizedPos, 4.2);
        const sSize = calculateDotSize(s.normalizedPos, 3.4);
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed top-0 left-0 w-full h-full -z-10 opacity-60 dark:opacity-50 pointer-events-none"
    />
  );
};
